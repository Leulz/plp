...
doOpAux lista listaTupla = if length lista == 0 then listaTupla else doOpAux (getFirstElement (countAndFilter (head lista) lista)) listaTupla ++ [((head lista, (getSecondElement (countAndFilter (head lista) lista))))]
